...
Partial Public Class SPAN_MbcPayment
	...
	Protected Sub ccsubmit_Click(sender As Object, e As EventArgs) Handles ccsubmit.Click
		...
		If CheckForOrder() Then	 'duplicate found
			...
			MsgBox1.Show("Duplicado", a, Nothing, New EO.Web.MsgBoxButton("Si", "", "ccyessubmit"), New EO.Web.MsgBoxButton("No", "", "nosubmit"))
		...
		Else
			...
			If CDec(total) > 0 Then
				...
				If authresponse.Approved Then
					...
					Session.Clear()
					Response.Redirect("SPAN_receipt.aspx?orderid=" & lblorderid.Text)

				Else 'card not approved

					Session("orderbad") = "TRUE"

					MsgBox1.Show("Error de envío Tarjeta de Crédito", "Su compañía de tarjeta de crédito devolvió el siguiente error:" & authresponse.Message & "Do you wish to try again?", Nothing, New EO.Web.MsgBoxButton("Yes", "", "yes"), New EO.Web.MsgBoxButton("No", "", "no"))
					If Session("numtrys") = 3 Then
						'they have had 3 tries call it  quits
						MsgBox1.Show("Error de envío Tarjeta de Crédito", "Su compañía de tarjeta de crédito devolvió el siguiente error:" & authresponse.Message & "Your order has not been processed. Please contact your Credit Card Company for help.", Nothing, New EO.Web.MsgBoxButton("OK"))
						DeleteOrder() 'deletes temporder
						Session.Abandon()
	...
	Protected Sub ecsubmit_Click(sender As Object, e As EventArgs) Handles ecsubmit.Click
		...
		If CheckForOrder() Then	 'duplicate found
			Dim a As String = Session("duporder")
			MsgBox1.Show("Duplicado", a, Nothing, New EO.Web.MsgBoxButton("Yes", "", "ecyessubmit"), New EO.Web.MsgBoxButton("No", "", "nosubmit"))
			Session.Remove("duporder")
		Else 'no duplicates go on
			StartECSubmission()
		End If
		...
		Else
			...
			If CDec(total) > 0 Then
				...
				Else 'card not approved
					...
					MsgBox1.Show("Verificar Error de envío", "El banco devolvió el siguiente error:" & authresponse.Message & "Do you wish to try again?", Nothing, New EO.Web.MsgBoxButton("Yes", "", "yes"), New EO.Web.MsgBoxButton("No", "", "no"))
					If Session("numtrys") = 3 Then
						'they have had 3 tries call it  quits
						MsgBox1.Show("Verificar Error de envío", "El banco devolvió el siguiente error:" & authresponse.Message & "Your order has not been processed. Please contact your Credit Card Company for help.", Nothing, New EO.Web.MsgBoxButton("OK"))
						DeleteOrder()
						Session.Abandon()
						Response.Redirect("http://www.memorybook.com/")
	...
	Protected Sub InsertRealOrder()
		...
		cmd.CommandText = "Insert into mbc.orders (orddate,yr,parentpayment,schname,orderid,grade,booktype,teacher,perstext1,perstext2,studentfname," _
		  & "studentlname,emailaddress,schcode,itemamount,itemtotal,itemqty,schinvoicenumber,sname,saddress,scity,sstate,szip) Values(Now(),@yr,1,@schname," _
		  ...
		  & "@schinvoicenumber,@sname,@saddress,@scity,@sstate,@szip);"
	...
	Private Function CheckForOrder() As Boolean
		...
		Try
			...
			Session.Add("duporder", "Nuestros registros indican que se han producido un pedido con este nombre del alumno (" & rstudfname & " " & rstudlname & ") on " & rddate.ToString _
			& " and paid for by " & rpayerfname & " " & rpayerlname & ". Haga clic en Sí para enviar el pago y realizar el pedido.")
		Catch ex As Exception
